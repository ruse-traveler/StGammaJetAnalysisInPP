...
static const Bool_t   DoWeighting = true;  // set to true for eT smearing
static const Bool_t   DoTrgNorm   = true;
static const UInt_t   TrgID       = 111;
static const Double_t PiValue     = TMath::Pi();

// filepaths
static const TString iPathDef("input/SeptRerunPi0/pp200py6et6pt12.forJetRerun_noCorruptTrees_run1t10.et6100pi0.r02rm1chrg.d30m9y2022.root");
static const TString wPathDef("input/ParticleGunWeights/triggerMatrix.forPaper_noShapeWeights_withWeightSmooth.et650x650vz55tsp008pi0.d22m11y2022.root");
static const TString oPathDef("pp200py6et6pt12.forWeightAndNormCheck_flatGunWeightIntEtNorm_pTbinOne.et911pi0.r02rm1chrg.d22m11y2022.root");

// parameters
static const Double_t MaxEta   = 0.9;
static const Double_t MinPtTrg = 6.;  // only need to change for unsmeared when looking at individual eTtrg bins
static const Double_t MaxPtTrg = 25.;  // only need to change for unsmeared when looking at individual eTtrg bins
...
static const Double_t MaxJetPt = 57.;
static const Double_t RecoilDf = PiValue / 4.;



void ReadJetTreeSimR02(const Bool_t inBatchMode=false, const TString iPath=iPathDef, const TString wPath=wPathDef, const TString oPath=oPathDef) {
  ...
  TH1D *hWeight;
  if (DoWeighting) {
    hWeight = (TH1D*) fWeight -> Get("hSmearWeight911");
    if (!hWeight) {
  ...
  JetTree -> SetBranchAddress("PtHat", &PtHat, &bPtHat);
  ...
  // pThat histograms [Derek, 10.01.2022, 10.23.2022]
  TH1D *hTrgPtHat[2];
  TH1D *hChkPtHat[2][4];
  TH1D *hTrgEtPtHat[2][4];
  TH1D *hTrgEtWeightPtHat[2][4];
  TH1D *hJetPtCorrPtHat[2][4];

  // pi0 pThat histograms [Derek, 10.01.2022, 10.23.2022]
  hTrgPtHat[0]            = new TH1D("hTrgPtHatP",               "Event #hat{p}_{T}, #pi^{0} trigger",                            eNum, eBins[0], eBins[1]);
  hChkPtHat[0][0]         = new TH1D("hChkPtHatP_pt47",          "#hat{p}_{T} check), #pi^{0} trig. (#hat{p}_{T} = 4 - 7 GeV/c)", eNum, eBins[0], eBins[1]);
  hChkPtHat[0][1]         = new TH1D("hChkPtHatP_pt79",          "#hat{p}_{T} check, #pi^{0} trig. (#hat{p}_{T} = 7 - 9 GeV/c)",  eNum, eBins[0], eBins[1]);
  hChkPtHat[0][2]         = new TH1D("hChkPtHatP_pt912",         "#hat{p}_{T} check, #pi^{0} trig. (#hat{p}_{T} = 9 - 12 GeV/c)", eNum, eBins[0], eBins[1]);
  hChkPtHat[0][3]         = new TH1D("hChkPtHatP_pt12",          "#hat{p}_{T} check, #pi^{0} trig. (#hat{p}_{T} > 12 GeV/c)",     eNum, eBins[0], eBins[1]);
  hTrgEtPtHat[0][0]       = new TH1D("hTrgEtPtHatP_pt47",        "Trigger E_{T}, #pi^{0} trig. (#hat{p}_{T} = 4 - 7 GeV/c)",      eNum, eBins[0], eBins[1]);
  hTrgEtPtHat[0][1]       = new TH1D("hTrgEtPtHatP_pt79",        "Trigger E_{T}, #pi^{0} trig. (#hat{p}_{T} = 7 - 9 GeV/c)",      eNum, eBins[0], eBins[1]);
  hTrgEtPtHat[0][2]       = new TH1D("hTrgEtPtHatP_pt912",       "Trigger E_{T}, #pi^{0} trig. (#hat{p}_{T} = 9 - 12 GeV/c)",     eNum, eBins[0], eBins[1]);
  hTrgEtPtHat[0][3]       = new TH1D("hTrgEtPtHatP_pt12",        "Trigger E_{T}, #pi^{0} trig. (#hat{p}_{T} > 12 GeV/c)",         eNum, eBins[0], eBins[1]);
  hTrgEtWeightPtHat[0][0] = new TH1D("hTrgEtWeightPtHatP_pt47",  "Trigger E_{T}, #pi^{0} trig. (#hat{p}_{T} = 4 - 7 GeV/c)",      eNum, eBins[0], eBins[1]);
  hTrgEtWeightPtHat[0][1] = new TH1D("hTrgEtWeightPtHatP_pt79",  "Trigger E_{T}, #pi^{0} trig. (#hat{p}_{T} = 7 - 9 GeV/c)",      eNum, eBins[0], eBins[1]);
  hTrgEtWeightPtHat[0][2] = new TH1D("hTrgEtWeightPtHatP_pt912", "Trigger E_{T}, #pi^{0} trig. (#hat{p}_{T} = 9 - 12 GeV/c)",     eNum, eBins[0], eBins[1]);
  hTrgEtWeightPtHat[0][3] = new TH1D("hTrgEtWeightPtHatP_pt12",  "Trigger E_{T}, #pi^{0} trig. (#hat{p}_{T} > 12 GeV/c)",         eNum, eBins[0], eBins[1]);
  if (UseVarBins) {
    hJetPtCorrPtHat[0][0] = new TH1D("hJetPtCorrP_pt47",  "Recoil jet p_{T}^{corr}, #pi^{0} trig. (#hat{p}_{T} = 4 - 7 GeV/c)",  pTnumVar, pTbinVar);
    hJetPtCorrPtHat[0][1] = new TH1D("hJetPtCorrP_pt79",  "Recoil jet p_{T}^{corr}, #pi^{0} trig. (#hat{p}_{T} = 7 - 9 GeV/c)",  pTnumVar, pTbinVar);
    hJetPtCorrPtHat[0][2] = new TH1D("hJetPtCorrP_pt912", "Recoil jet p_{T}^{corr}, #pi^{0} trig. (#hat{p}_{T} = 9 - 12 GeV/c)", pTnumVar, pTbinVar);
    hJetPtCorrPtHat[0][3] = new TH1D("hJetPtCorrP_pt12",  "Recoil jet p_{T}^{corr}, #pi^{0} trig. (#hat{p}_{T} > 12 GeV/c)",     pTnumVar, pTbinVar);
  } else {
    hJetPtCorrPtHat[0][0] = new TH1D("hJetPtCorrP_pt47",  "Recoil jet p_{T}^{corr}, #pi^{0} trig. (#hat{p}_{T} = 4 - 7 GeV/c)",  pTnum, pTbins[0], pTbins[1]);
    hJetPtCorrPtHat[0][1] = new TH1D("hJetPtCorrP_pt79",  "Recoil jet p_{T}^{corr}, #pi^{0} trig. (#hat{p}_{T} = 7 - 9 GeV/c)",  pTnum, pTbins[0], pTbins[1]);
    hJetPtCorrPtHat[0][2] = new TH1D("hJetPtCorrP_pt912", "Recoil jet p_{T}^{corr}, #pi^{0} trig. (#hat{p}_{T} = 9 - 12 GeV/c)", pTnum, pTbins[0], pTbins[1]);
    hJetPtCorrPtHat[0][3] = new TH1D("hJetPtCorrP_pt12",  "Recoil jet p_{T}^{corr}, #pi^{0} trig. (#hat{p}_{T} > 12 GeV/c)",     pTnum, pTbins[0], pTbins[1]);
  }
  // gamma pThat histograms [Derek, 10.01.2022, 10.23.2022]
  hTrgPtHat[1]            = new TH1D("hTrgPtHatG",               "Event #hat{p}_{T}, #gamma trigger",                            eNum, eBins[0], eBins[1]);
  hChkPtHat[1][0]         = new TH1D("hChkPtHatG_pt47",          "#hat{p}_{T} check, #gamma trig. (#hat{p}_{T} = 4 - 7 GeV/c)",  eNum, eBins[0], eBins[1]);
  hChkPtHat[1][1]         = new TH1D("hChkPtHatG_pt79",          "#hat{p}_{T} check, #gamma trig. (#hat{p}_{T} = 7 - 9 GeV/c)",  eNum, eBins[0], eBins[1]);
  hChkPtHat[1][2]         = new TH1D("hChkPtHatG_pt912",         "#hat{p}_{T} check, #gamma trig. (#hat{p}_{T} = 9 - 12 GeV/c)", eNum, eBins[0], eBins[1]);
  hChkPtHat[1][3]         = new TH1D("hChkPtHatG_pt12",          "#hat{p}_{T} check, #gamma trig. (#hat{p}_{T} > 12 GeV/c)",     eNum, eBins[0], eBins[1]);
  hTrgEtPtHat[1][0]       = new TH1D("hTrgEtPtHatG_pt47",        "Trigger E_{T}, #gamma trig. (#hat{p}_{T} = 4 - 7 GeV/c)",      eNum, eBins[0], eBins[1]);
  hTrgEtPtHat[1][1]       = new TH1D("hTrgEtPtHatG_pt79",        "Trigger E_{T}, #gamma trig. (#hat{p}_{T} = 7 - 9 GeV/c)",      eNum, eBins[0], eBins[1]);
  hTrgEtPtHat[1][2]       = new TH1D("hTrgEtPtHatG_pt912",       "Trigger E_{T}, #gamma trig. (#hat{p}_{T} = 9 - 12 GeV/c)",     eNum, eBins[0], eBins[1]);
  hTrgEtPtHat[1][3]       = new TH1D("hTrgEtPtHatG_pt12",        "Trigger E_{T}, #gamma trig. (#hat{p}_{T} > 12 GeV/c)",         eNum, eBins[0], eBins[1]);
  hTrgEtWeightPtHat[1][0] = new TH1D("hTrgEtWeightPtHatG_pt47",  "Trigger E_{T}, #gamma trig. (#hat{p}_{T} = 4 - 7 GeV/c)",      eNum, eBins[0], eBins[1]);
  hTrgEtWeightPtHat[1][1] = new TH1D("hTrgEtWeightPtHatG_pt79",  "Trigger E_{T}, #gamma trig. (#hat{p}_{T} = 7 - 9 GeV/c)",      eNum, eBins[0], eBins[1]);
  hTrgEtWeightPtHat[1][2] = new TH1D("hTrgEtWeightPtHatG_pt912", "Trigger E_{T}, #gamma trig. (#hat{p}_{T} = 9 - 12 GeV/c)",     eNum, eBins[0], eBins[1]);
  hTrgEtWeightPtHat[1][3] = new TH1D("hTrgEtWeightPtHatG_pt12",  "Trigger E_{T}, #gamma trig. (#hat{p}_{T} > 12 GeV/c)",         eNum, eBins[0], eBins[1]);
  if (UseVarBins) {
    hJetPtCorrPtHat[1][0] = new TH1D("hJetPtCorrG_pt47",  "Recoil jet p_{T}^{corr}, #gamma trig. (#hat{p}_{T} = 4 - 7 GeV/c)",  pTnumVar, pTbinVar);
    hJetPtCorrPtHat[1][1] = new TH1D("hJetPtCorrG_pt79",  "Recoil jet p_{T}^{corr}, #gamma trig. (#hat{p}_{T} = 7 - 9 GeV/c)",  pTnumVar, pTbinVar);
    hJetPtCorrPtHat[1][2] = new TH1D("hJetPtCorrG_pt912", "Recoil jet p_{T}^{corr}, #gamma trig. (#hat{p}_{T} = 9 - 12 GeV/c)", pTnumVar, pTbinVar);
    hJetPtCorrPtHat[1][3] = new TH1D("hJetPtCorrG_pt12",  "Recoil jet p_{T}^{corr}, #gamma trig. (#hat{p}_{T} > 12 GeV/c)",     pTnumVar, pTbinVar);
  } else {
    hJetPtCorrPtHat[1][0] = new TH1D("hJetPtCorrG_pt47",  "Recoil jet p_{T}^{corr}, #gamma trig. (#hat{p}_{T} = 4 - 7 GeV/c)",  pTnum, pTbins[0], pTbins[1]);
    hJetPtCorrPtHat[1][1] = new TH1D("hJetPtCorrG_pt79",  "Recoil jet p_{T}^{corr}, #gamma trig. (#hat{p}_{T} = 7 - 9 GeV/c)",  pTnum, pTbins[0], pTbins[1]);
    hJetPtCorrPtHat[1][2] = new TH1D("hJetPtCorrG_pt912", "Recoil jet p_{T}^{corr}, #gamma trig. (#hat{p}_{T} = 9 - 12 GeV/c)", pTnum, pTbins[0], pTbins[1]);
    hJetPtCorrPtHat[1][3] = new TH1D("hJetPtCorrG_pt12",  "Recoil jet p_{T}^{corr}, #gamma trig. (#hat{p}_{T} > 12 GeV/c)",     pTnum, pTbins[0], pTbins[1]);
  }

  // pThat histogram errors [Derek, 10.01.2022]
  for (UInt_t iTrgType = 0; iTrgType < 2; iTrgType++) {
    hTrgPtHat[iTrgType] -> Sumw2();
    for (UInt_t iPtHat = 0; iPtHat < 4; iPtHat++) {
      hChkPtHat[iTrgType][iPtHat]         -> Sumw2();
      hTrgEtPtHat[iTrgType][iPtHat]       -> Sumw2();
      hTrgEtWeightPtHat[iTrgType][iPtHat] -> Sumw2();
      hJetPtCorrPtHat[iTrgType][iPtHat]   -> Sumw2();
    }
  }
  ...
  Int_t    nTrgPi0(0);
  Int_t    nTrgGam(0);
  Double_t nWeightPi0(0.);
  Double_t nWeightGam(0.);
  for (Int_t i = 0; i < nEvts; i++) {
    ...
    const Double_t pThat = PtHat;

    const Bool_t isInTrgEtaCut = (TMath::Abs(hTrg) < MaxEta);
    const Bool_t isInTrgEtCut  = ((eTtrg > MinPtTrg) && (eTtrg < MaxPtTrg));
    ...
    // determine pThat bins [Derek, 10.01.2022]
    UInt_t binPtHat      = 5;
    Bool_t isInPtHatBin1 = ((pThat >= 4.) && (pThat < 7.));
    Bool_t isInPtHatBin2 = ((pThat >= 7.) && (pThat < 9.));
    Bool_t isInPtHatBin3 = ((pThat >= 9.) && (pThat < 12.));
    Bool_t isInPtHatBin4 = (pThat >= 12.);
    if (isInPtHatBin1) {
      binPtHat = 0;
    } else if (isInPtHatBin2) {
      binPtHat = 1;
    } else if (isInPtHatBin3) {
      binPtHat = 2;
    } else if (isInPtHatBin4) {
      binPtHat = 3;
    } else {
      binPtHat = 0;
    }
    ...
    // fill pThat histograms [Derek, 10.01.2022, 10.23.2022]
    if (tspFlag == 0) {
      hTrgPtHat[0]             -> Fill(pThat);
      hChkPtHat[0][binPtHat]   -> Fill(pThat);
      hTrgEtPtHat[0][binPtHat] -> Fill(TrgEt);
      if (DoWeighting) {
        hTrgEtWeightPtHat[0][binPtHat] -> Fill(TrgEt, eTweight);
      } else {
        hTrgEtWeightPtHat[0][binPtHat] -> Fill(TrgEt);
      }
    } else if (tspFlag == 1) {
      hTrgPtHat[1]             -> Fill(pThat);
      hChkPtHat[1][binPtHat]   -> Fill(pThat);
      hTrgEtPtHat[0][binPtHat] -> Fill(TrgEt);
      if (DoWeighting) {
        hTrgEtWeightPtHat[0][binPtHat] -> Fill(TrgEt, eTweight);
      } else {
    ...
    for (Int_t j = 0; j < nJets; j++) {
      ...
      // fill pThat histograms [Derek, 10.01.2022]
      if (DoWeighting) {
        if (tspFlag == 0) {
          hJetPtCorrPtHat[0][binPtHat] -> Fill(pTc, eTweight);
        } else {
          hJetPtCorrPtHat[1][binPtHat] -> Fill(pTc, eTweight);
        }
      } else {
        if (tspFlag == 0) {
          hJetPtCorrPtHat[0][binPtHat] -> Fill(pTc);
        } else {
          hJetPtCorrPtHat[1][binPtHat] -> Fill(pTc);
        }
      }
    }  // end jet loop
  ...
  // for normalization of weighted jets
  const Double_t intEtWeightPi0 = hTrgEtWeight[0] -> Integral();
  const Double_t intEtWeightGam = hTrgEtWeight[1] -> Integral();
  if (DoWeighting) {
    cout << "  Calculated integrals for normalization:\n"
         << "    intEtWeightPi0 = " << intEtWeightPi0 << "\n"
         << "    intEtWeightGam = " << intEtWeightGam
         << endl;
  }
    ...
    if (DoTrgNorm) {
      if (DoWeighting) {
        if (i == 0) {
          ...
          normW = intEtWeightPi0;
        } else if (i == 1) {
          norm  = (Double_t) nTrgGam;
          normW = intEtWeightGam;
        }
      } else {
        if (i == 0)  {
          norm  = (Double_t) nTrgPi0;
  ...
  for (Int_t i = 0; i < 2; i++) {
    ...
    hJetPtVsDf[i]      -> Write();
    hJetPtVsA[i]       -> Write();
    // save pThat histograms [Derek, 10.01.2022]
    hTrgPtHat[i]       -> Write();
    for (UInt_t iPtHat = 0; iPtHat < 4; iPtHat++) {
      hChkPtHat[i][iPtHat]         -> Write();
      hTrgEtPtHat[i][iPtHat]       -> Write();
      hTrgEtWeightPtHat[i][iPtHat] -> Write();
      hJetPtCorrPtHat[i][iPtHat]   -> Write();
    }
    fOut -> cd();
  }
  fOut -> Close();
