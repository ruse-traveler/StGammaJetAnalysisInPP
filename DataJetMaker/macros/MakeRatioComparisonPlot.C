...
void MakeRatioComparisonPlot() {
  ...
  // file parameters
  const TString sOutput("rhoVsOffAxis.ratioToRaw_forThesis.et911r05pi0.d20m2y2022.root");
  const TString sInDenom[NHist] = {"pp_JetRecoHist_0p2to30trk_NojetCut_DerekTreeMain_OA_R5_HrdJetRmv1_Pt9to11_d4m1y2022.root",
                                   "pp_JetRecoHist_0p2to30trk_NojetCut_DerekTreeMain_OA_R5_HrdJetRmv1_Pt9to11_d4m1y2022.root",
                                   "pp_JetRecoHist_0p2to30trk_NojetCut_DerekTreeMain_OA_R5_HrdJetRmv1_Pt9to11_d4m1y2022.root"};
  const TString sInNumer[NHist] = {"pp_JetRecoHist_0p2to30trk_NojetCut_DerekTreeMain_OA_R5_HrdJetRmv1_Pt9to11_d4m1y2022.root",
                                   "pp_JetRecoHist_0p2to30trk_NojetCut_DerekTreeMain_OA_R5_HrdJetRmv1_Pt9to11_d4m1y2022.root",
                                   "pp_JetRecoHist_0p2to30trk_NojetCut_DerekTreeMain_OA_R5_HrdJetRmv1_Pt9to11_d4m1y2022.root"};

  // denominator parameters
  const TString sHeadDenom("");
  const TString sNameDenom[NHist]  = {"hJetPtRaw", "hJetPtRaw", "hJetPtRaw"};
  const TString sHistDenom[NHist]  = {"h_JetPt_pi0_Recoil",
                                      "h_JetPt_pi0_Recoil",
                                      "h_JetPt_pi0_Recoil"};
  const TString sLabelDenom[NHist] = {"Raw p_{T}^{jet}",
                                      "Raw p_{T}^{jet}",
                                      "Raw p_{T}^{jet}"};

  // numerator parameters
  const TString sHeadNumer("");
  const TString sNameNumer[NHist]  = {"hJetPtRhoA", "hJetOA", "hJetPtOA"};
  const TString sHistNumer[NHist]  = {"h_JetPtCorr_pi0_Recoil",
                                      "h_JetPtOA_pi0_Recoil",
                                      "h_JetPtOACorr_pi0_Recoil"};
  const TString sLabelNumer[NHist] = {"p_{T}^{reco} = p_{T}^{jet} - (#rho #upoint A^{jet})",
                                      "Off-Axis p_{T}, p_{T}^{OA}",
                                      "p_{T}^{reco} = p_{T}^{jet} - p_{T}^{OA}"};

  // plot parameters
  const TString sTitle("");
  const TString sTitleX("p_{T} [GeV/c]");
  const TString sTitleY("(1/N_{trg}) dN_{jet}^{3}/d(p_{T} #eta_{jet}) [GeV/c]^{-1}");
  const TString sTitleR("p_{T}^{reco} / p_{T}^{jet}");
  const TString sNameRatio[NHist] = {"hRatioPtRhoA", "hRatioOA", "hRatioPtOA"};
  const TString sText[NTxt]       = {"pp-collisions, #sqrt{s} = 200 GeV",
                                     "#pi^{0} trig., E_{T}^{trg} #in (9, 11) GeV",
                                     "anti-k_{T}, R = 0.5",
                                     "#bf{charged jets}"};
  const Float_t xPlotRange[NPlot] = {-3., 30.};
  const UInt_t  fColDen[NHist]    = {923, 923, 923};
  const UInt_t  fColNum[NHist]    = {859, 839, 819};
  const UInt_t  fMarDen[NHist]    = {20, 20, 20};
  const UInt_t  fMarNum[NHist]    = {26, 25, 32};
  ...
  leg -> AddEntry(hDenom[0], sLabelDenom[0].Data(), "pf");
  leg -> AddEntry(hNumer[0], sLabelNumer[0].Data(), "pf");
  leg -> AddEntry(hNumer[1], sLabelNumer[1].Data(), "pf");
  leg -> AddEntry(hNumer[2], sLabelNumer[2].Data(), "pf");
/*
...
*/
  ...
  // QUICK FIX [02.20.2022]
  TLegend *legR = new TLegend(0.1, 0.1, 0.3, 0.26);
  ...
  legR -> AddEntry(hRatio[0], sLabelNumer[0].Data(), "pf");
  legR -> AddEntry(hRatio[2], sLabelNumer[2].Data(), "pf");
