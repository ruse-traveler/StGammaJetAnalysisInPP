...
static const TString SParDefault("../../../JetReco_pp/PythiaJetMaker/output/pp200py8pt2025par.forUnfolding.et920nTrg50Kpt021Kpi0.r02rm1chrg.d5m3y2020.root");
static const TString SDetDefault("../../../JetReco_pp/PythiaJetMaker/output/pp200py8pt2025det.forUnfolding.et920nTrg50Kpt0230pi0.r02rm1chrg.d5m3y2020.root");
...
void MatchPythiaJets(const TString pPath=SParDefault, const TString dPath=SDetDefault, const TString oPath=SOutDefault, Bool_t inBatchMode=false) {
  ...
  // candidate match jet qT [04.17.2020]
  TH1D *hJetQt_pt021 = new TH1D("hJetQt_pt021", "Matched jet q_{T}, p_{T}^{det} #in (0.2, 1) GeV/c", qNum, qBin[0], qBin[1]);
  TH1D *hJetQt_pt12  = new TH1D("hJetQt_pt12", "Matched jet q_{T}, p_{T}^{det} #in (1, 2) GeV/c", qNum, qBin[0], qBin[1]);
  TH1D *hJetQt_pt25  = new TH1D("hJetQt_pt25", "Matched jet q_{T}, p_{T}^{det} #in (2, 5) GeV/c", qNum, qBin[0], qBin[1]);
  TH1D *hJetQt_pt510 = new TH1D("hJetQt_pt510", "Matched jet q_{T}, p_{T}^{det} #in (5, 10) GeV/c", qNum, qBin[0], qBin[1]);
  TH1D *hJetQt_pt10  = new TH1D("hJetQt_pt10", "Matched jet q_{T}, p_{T}^{det} > 10 GeV/c", qNum, qBin[0], qBin[1]);
  hJetQt_pt021 -> Sumw2();
  hJetQt_pt12  -> Sumw2();
  hJetQt_pt25  -> Sumw2();
  hJetQt_pt510 -> Sumw2();
  ...
  for (UInt_t i = 0; i < nEvts; i++) {
    ...
    for (UInt_t j = 0; j < nPjets; j++) {
      ...
      for (UInt_t k = 0; k < nDjets; k++) {
        ...
        // candidate match jet qT [04.14.2020]
        if ((dPt >= 0.2) && (dPt < 1.)) {
          hJetQt_pt021 -> Fill(qT);
        }
        if ((dPt >= 1.) && (dPt < 2.)) {
          hJetQt_pt12  -> Fill(qT);
        }
