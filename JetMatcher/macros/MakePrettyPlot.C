...
void MakePrettyPlot() {
  ...
  const TString sOutput("fakeJetCheck.jetPt.et911x1115x1520r02pi0.d13m6y2022.root");
  const TString sDenom("fakeJetCheck.jetPt.et911r02qt05130.d13m6y2022.root");
  const TString sHistDenom("hSum");
  const TString sNameDenom("hFakeJetPt_et911r02");
  const TString sLabelDenom("E_{T}^{trg} #in (9, 11) GeV");

  // numerator parameters
  const TString sNumer[NNumer]      = {"fakeJetCheck.jetPt.et1115r02qt05130.d13m6y2022.root",
                                       "fakeJetCheck.jetPt.et1520r02qt05130.d13m6y2022.root"};
  const TString sHistNumer[NNumer]  = {"hSum", "hSum"};
  const TString sNameNumer[NNumer]  = {"hFakeJetPt_et1115r02", "hFakeJetPt_et1520r02"};
  const TString sNameRatio[NNumer]  = {"hFakeJetPtRatio_et1115x911r02", "hFakeJetPtRatio_et1520x911r02"};
  const TString sLabelNumer[NNumer] = {"E_{T}^{trg} #in (11, 15) GeV", "E_{T}^{trg} #in (15, 20) GeV"};

  // plot parameters
  const TString sTitle("");
  const TString sTitleX("p_{T}^{reco} = p_{T}^{jet} - (#rho #upoint A^{jet}) [GeV/c]");
  const TString sTitleY("(1 / N^{trg}) dN^{fake}/(dp_{T}^{reco} d#eta^{jet}) [GeV/c]^{-1}");
  const TString sTitleR("ratio to (9, 11) GeV");
  const TString sOptDenom("");
  const TString sOptNumer("same");
  const TString sOptRatio("");
  const Float_t xPlotRange[NPlot] = {0., 30.};
  const UInt_t  fColDen(634);
  const UInt_t  fMarDen(20);
  const UInt_t  fColNum[NNumer] = {602, 418};
  const UInt_t  fMarNum[NNumer] = {21, 22};
  ...
  const TString sHeader("#bf{fake jets}");

  // open output and denominator files
  ...
  if (DoRebin) {
    for (UInt_t iNumer = 0; iNumer < NNumer; iNumer++) {
      ...
      hNumer[iNumer] -> Scale(1. / ((Double_t) NRebin));
    }
    hDenom -> Rebin(NRebin);
    hDenom -> Scale(1. / ((Double_t) NRebin));
  ...
  const UInt_t  widthNoRatio(950);
  const UInt_t  heightNoRatio(950);
  ...
  const Float_t fMarginL1(0.15);
  const Float_t fMarginL2(0.13);
  ...
  const Float_t fMarginB3(0.13);
  const Float_t fPadXY1[NVtx] = {0., 0., 1., 0.35};
  const Float_t fPadXY2[NVtx] = {0., 0.35, 1., 1.};

  TString sSameOptR(sOptRatio.Data());
  sSameOptR.Append(" same");
  ...
  pPad2     -> Draw();
  pPad1     -> cd();
  hRatio[0] -> Draw(sOptRatio.Data());
  for (UInt_t iNumer = 1; iNumer < NNumer; iNumer++) {
    hRatio[iNumer] -> Draw(sSameOptR.Data());
  }
  line   -> Draw();
  pPad2  -> cd();
  hDenom -> Draw(sOptDenom.Data());
  for(UInt_t iNumer = 0; iNumer < NNumer; iNumer++) {
    hNumer[iNumer] -> Draw(sOptNumer.Data());
  }
  leg     -> Draw();
  txt     -> Draw();
  fOutput -> cd();
  ...
  TCanvas *cNoRatio = new TCanvas("cPlotWithNoRatio", "", widthNoRatio, heightNoRatio);
  cNoRatio -> SetGrid(fGrid, fGrid);
  cNoRatio -> SetTicks(fTick, fTick);
  cNoRatio -> SetLogx(fLogX);
  cNoRatio -> SetLogy(fLogY2);
  cNoRatio -> SetBorderMode(fMode);
  cNoRatio -> SetBorderSize(fBord);
  cNoRatio -> SetFrameBorderMode(fFrame);
  cNoRatio -> SetLeftMargin(fMarginL2);
  cNoRatio -> SetRightMargin(fMarginR);
  cNoRatio -> SetTopMargin(fMarginT2);
  cNoRatio -> SetBottomMargin(fMarginB3);
  cNoRatio -> cd();
  hDenom   -> Draw(sOptDenom.Data());
  for(UInt_t iNumer = 0; iNumer < NNumer; iNumer++) {
    hNumer[iNumer] -> Draw(sOptNumer.Data());
