...
void CompareResponse() {
  ...
  // i/o parameters
  //const TString sOutput("inputVsOutputMatrix_quickResponseSmoothTest_fromEmbed.et911r05qt05130.d3m6y2021.root");
  const TString sOutput("test.root");
  const TString sInput[NHist] = {"quickResponseSmoothTest_fromEmbed.et911r05qt05130.d3m6y2021.root", "quickResponseSmoothTest_fromEmbed.et911r05qt05130.d3m6y2021.root"};
  const TString sHist[NHist]  = {"hResponseInput", "hResponseOutput"};

  // hist parameters
  const TString sRatio("|input - output| / input");
  const TString sRatioName("hRatioInputToOutput");
  const TString sTitle[NHist] = {"Input Py6#oplusGeant R_{ij}", "Smoothed Py6#oplusGeant R_{ij}"};
  const TString sNameH[NHist] = {"hResponseIn", "hResponseOut"};
  const TString sNameP[NHist] = {"pResponseIn", "pResponseOut"};
  const TString sAxis[NAxes]  = {"p_{T}^{det} [GeV/c]", "p_{T}^{par} [GeV/c]"};
  const TString sLabel[NHist] = {"Input Py6#oplusGeant R_{ij}", "Smoothed Py6#oplusGeant R_{ij}"};
  ...
  // profile parameters
  const TString sProfTitle("Input vs. output response profiles");
