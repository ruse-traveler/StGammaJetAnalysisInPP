...
void MakeSummaryPlot_ForQM22gam() {
  ...
  // io parameters
  const TString sOut("gammaSummaryPP.onlyPy8_forQM22_betterRes_pTbinHuge.et920r05gam.d2m4y2022.root");
  ...
  const TString sInStat[NTrgTot] = {"et911r05gam_forQM22/statErr.shiftedDataPoints_forQM22_pTbinHuge.et911r05gam.d27m3y2022.root",
                                    "et1115r05gam_forQM22/statErr.shiftedDataPoints_forQM22_pTbinHuge.et1115r05gam.d27m3y2022.root",
                                    "et1520r05gam_forQM22/statErr.shiftedDataPoints_forQM22_pTbinHuge.et1520r05gam.d27m3y2022.root"};
  const TString sInUnf[NTrgTot]  = {"et911r05gam_forQM22/sysErr.shiftedDataPoints_forQM22_pTbinHuge.et911r05gam.d27m3y2022.root",
                                    "et1115r05gam_forQM22/sysErr.shiftedDataPoints_forQM22_pTbinHuge.et1115r05gam.d27m3y2022.root",
                                    "et1520r05gam_forQM22/sysErr.shiftedDataPoints_forQM22_pTbinHuge.et1520r05gam.d27m3y2022.root"};
  const TString sInPyF[NTrgTot]  = {"et911r05gam_forQM22/pythia.shiftedDataPoints_forQM22_pTbinHuge.et6100x911r05gam.d28m3y2022.root",
                                    "et1115r05gam_forQM22/pythia.shiftedDataPoints_forQM22_pTbinHuge.et6100x1115r05gam.d28m3y2022.root",
                                    "et1520r05gam_forQM22/pythia.shiftedDataPoints_forQM22_pTbinHuge.et6100x1520r05gam.d28m3y2022.root"};
  const TString sInPyH[NTrgTot]  = {"et911r05gam_forQM22/pythia.shiftedDataPoints_forQM22_pTbinHuge.et6100x911r05gam.d28m3y2022.root",
                                    "et1115r05gam_forQM22/pythia.shiftedDataPoints_forQM22_pTbinHuge.et6100x1115r05gam.d28m3y2022.root",
                                    "et1520r05gam_forQM22/pythia.shiftedDataPoints_forQM22_pTbinHuge.et6100x1520r05gam.d28m3y2022.root"};

  // text parameters
  const TString sSys("p+p #sqrt{s} = 200 GeV");
  const TString sTyp("#gamma_{dir}+jet, anti-k_{T}");
  ...
  const TString sNameStat[NTrgTot] = {"gStat911ga",     "gStat1115ga",     "gStat1520ga"};
  const TString sNameUnf[NTrgTot]  = {"gSys911ga",      "gSys1115ga",      "gSys1520ga"};
  const TString sPlotStat[NTrgTot] = {"gPlotStat911ga", "gPlotStat1115ga", "gPlotStat1520ga"};
  const TString sPlotUnf[NTrgTot]  = {"gPlotSys911ga",  "gPlotSys1115ga",  "gPlotSys1520ga"};
  const TString sLabels[NTrgTot]   = {"9 < E_{T}^{trig} < 11 GeV",
                                      "11 < E_{T}^{trig} < 15 GeV",
                                      "15 < E_{T}^{trig} < 20 GeV"};

  // trigger and jet parameters
  const UInt_t  fColSt[NJetRes][NTrgTot]      = {{418, 634, 602}, {418, 634, 602}};
  const UInt_t  fColUn[NJetRes][NTrgTot]      = {{407, 623, 591}, {407, 623, 591}};
  const UInt_t  fColOu[NJetRes][NTrgTot]      = {{418, 634, 602}, {418, 634, 602}};
  const UInt_t  fMarSt[NJetRes][NTrgTot]      = {{1, 1, 1}, {1, 1, 1}};
  const UInt_t  fMarUn[NJetRes][NTrgTot]      = {{1, 1, 1}, {1, 1, 1}};
  const UInt_t  fMarOu[NJetRes][NTrgTot]      = {{1, 1, 1}, {1, 1, 1}};
  const UInt_t  nJetPtBins[NJetRes][NTrgTot]  = {{3, 4, 5}, {3, 4, 5}};
  const Bool_t  goesToZero[NJetRes][NTrgTot]  = {{false, false, false}, {false, false, false}};
  const Float_t xMaxJetBins[NJetRes][NTrgTot] = {{11., 15., 20.}, {11., 15., 20.}};
  const Float_t fTrgScale[NJetRes][NTrgTot]   = {{1., 1., 1.},   {1., 1., 1.}};

  // pythia parameters
  const TString sNamePyF[NTrgTot]         = {"gPyFit911ga",      "gPyFit1115ga",      "gPyFit1115ga"};
  const TString sNamePyH[NTrgTot]         = {"gPyHist911ga",     "gPyHist1115ga",     "gPyHist1115ga"};
  const TString sPlotPyF[NTrgTot]         = {"gPlotPyFit911ga",  "gPlotPyFit1115ga",  "gPlotPyFit1520ga"};
  const TString sPlotPyH[NTrgTot]         = {"gPlotPyHist911ga", "gPlotPyHist1115ga", "gPlotPyHist1520ga"};
  const UInt_t  fColPyF[NJetRes][NTrgTot] = {{819, 899, 859}, {819, 899, 859}};
  const UInt_t  fColPyH[NJetRes][NTrgTot] = {{819, 899, 859}, {819, 899, 859}};
  const UInt_t  fMarPyF[NJetRes][NTrgTot] = {{1, 1, 1}, {1, 1, 1}};
  const UInt_t  fMarPyH[NJetRes][NTrgTot] = {{1, 1, 1}, {1, 1, 1}};
...
*/
  ...
  leg -> AddEntry(gDataLeg[2], sLabels[2].Data(), "lf");
