...
void MakeRatioComparisonPlot() {
  ...
  // file parameters
  const TString sOutput("pp200py8.weightFromMatrixVsQt_pTbinOne.et920r02gam.d8m3y2022.root");
  const TString sInDenom[NHist] = {"output/March2022/pp200py8par.forEtCheck_withWeights_pTbinOne.et911r02gam.d8m3y2022.root",
                                   "output/March2022/pp200py8par.forEtCheck_withWeights_pTbinOne.et1115r02gam.d8m3y2022.root",
                                   "output/March2022/pp200py8par.forEtCheck_withWeights_pTbinOne.et1520r02gam.d8m3y2022.root"};
  const TString sInNumer[NHist] = {"input/Weights/weightedPythia8.et650x920gam.d8m3y2022.root",
                                   "input/Weights/weightedPythia8.et650x920gam.d8m3y2022.root",
                                   "input/Weights/weightedPythia8.et650x920gam.d8m3y2022.root"};

  // denominator parameters
  const TString sHeadDenom("#bf{Weights From q_{T}^{trg}}");
  const TString sHistDenom[NHist]  = {"Gam/hTrgEtWeightG", "Gam/hTrgEtWeightG", "Gam/hTrgEtWeightG"};
  const TString sNameDenom[NHist]  = {"hQtWeight911", "hQtWeight1115", "hQtWeight1520"};
  const TString sLabelDenom[NHist] = {"9 < E_{T}^{select} < 11 GeV",
                                      "11 < E_{T}^{select} < 15 GeV",
                                      "15 < E_{T}^{select} < 20 GeV"};

  // numerator parameters
  const TString sHeadNumer("#bf{Weights From Matrix}");
  const TString sHistNumer[NHist]  = {"hWeightedTrigEt911", "hWeightedTrigEt1115", "hWeightedTrigEt1520"};
  const TString sNameNumer[NHist]  = {"hMatrixWeight911", "hMatrixWeight1115", "hMatrixWeight1520"};
  const TString sLabelNumer[NHist] = {"9 < E_{T}^{select} < 11 GeV",
                                      "11 < E_{T}^{select} < 15 GeV",
                                      "15 < E_{T}^{select} < 20 GeV"};

  // plot parameters
  const TString sTitle("");
  const TString sTitleX("E_{T}^{true} [GeV]");
  const TString sTitleY("a. u.");
  const TString sTitleR("q_{T}^{trg} / matrix");
  const TString sNameRatio[NHist] = {"hRatio911", "hRatio1115", "hRatio1520"};
  const TString sText[NTxt]       = {"PYTHIA-8", "#gamma_{dir}+jet, p+p 200 GeV", ""};
  const Float_t xPlotRange[NPlot] = {3., 30.};
  const UInt_t  fColDen[NHist]    = {803, 893, 883};
  const UInt_t  fColNum[NHist]    = {799, 899, 879};
  ...
  if (DoIntNorm) {
    for (UInt_t iHist = 0; iHist < NHist; iHist++) {
      const Double_t intDen = hDenom[iHist] -> Integral();
      const Double_t intNum = hNumer[iHist] -> Integral();
      hDenom[iHist] -> Scale(1. / intDen);
      hNumer[iHist] -> Scale(1. / intNum);
    }
