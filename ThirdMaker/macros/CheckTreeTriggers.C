...
// by the 'StJetTreeThirdMaker' class.
...
// i/o parameters
static const TString sIn("merge.beforeFix.r10145070.d12m4y2017.root");
static const TString sOut("treeTrigs.beforeFix.r10145070.d12m4y2017.root");
static const TString sTree("Gfmtodst");


void CheckTreeTriggers() {
  ...
  Int_t    nTrigPass = 0;
  Int_t    nTrigDiff = 0;
  Int_t    nTrgTot   = tTree -> GetEntriesFast();
  ...
  for (Int_t i = 0; i < nTrgTot; i++) {
    ...
    // event cuts
    Bool_t isInStrCut  = ((EEstrpen4 >= eStrMin) && (EPstripenp4 >= pStrMin));
    Bool_t isInTwrCut  = ((ETwreT > eTwrMin) || (ETwreT < eTwrMax));
    Bool_t isInAdcCut  = (ETwradc11 <= adcMax);
    Bool_t isInPTwrCut = (ETwrPTower < pTwrMax);
    if (!isInStrCut)
      continue;
    if (!isInTwrCut)
      continue;
    ...
    Bool_t isInTspPCut = ((Etsp > tspMinP) && (Etsp < tspMaxP));
    Bool_t isInTspGCut = ((Etsp > tspMinG) && (Etsp < tspMaxG));
    if (!isInEtaCut)
      continue;
    if (!isInEneCut)
      continue;
