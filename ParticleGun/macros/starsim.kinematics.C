...
void trig( Int_t n=1 )
{
  for ( Int_t i=0; i<n; i++ ) {
   ...
   /* kinematics->Dist(4, "pi0", ptDist, etaDist );
    ...
    kinematics->Dist(4, "pi+", ptDist, etaDist );

    kinematics->Dist(4, "pi-", ptDist, etaDist );*/
    kinematics->Kine(50, "pi0", 11.0, 15.0, -0.9, 0.9 );

    kinematics->Kine(2, "pi+", 0.0, 3.0, -1.0, 1.0 );

    //kinematics->Kine(2, "pi-", 0.0, 15.0, -1.0, 1.0 );
...
// ----------------------------------------------------------------------------
// ----------------------------------------------------------------------------
void starsim( Int_t nevents=10, Int_t rngSeed=1234, TString outname = "")
{ 

  gROOT->ProcessLine(".L bfc.C");
  {
    //TString simple = "y2014b geant gstar usexgeom agml ";
    TString simple = "y2009d geant gstar usexgeom agml ";  // TEST [Derek, 06.23.2020]
    bfc(0, simple );
  }
  ...
  //
  //geometry("y2014b");
  geometry("y2009d");  // TEST [Derek, 06.23.2020]
  command("gkine -4 0");
